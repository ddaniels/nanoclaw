...
export async function restoreArchive(opts: RestoreOptions): Promise<RestoreResult> {
  // Dry-run never mutates state, so it's safe to inspect even with a live host.
  // Running it against a live install is in fact useful — operators can preview
  // what a restore would do before deciding to take the service down.
  if (!opts.dryRun) {
