...
```bash
git fetch fork skill/signal
```

### 2. Copy the adapter and tests

```bash
git show fork/skill/signal:src/channels/signal.ts > src/channels/signal.ts
git show fork/skill/signal:src/channels/signal.test.ts > src/channels/signal.test.ts
```
...
```env
# Path to signal-cli binary (default: "signal-cli", found via PATH)
...
Run `/manage-channels` to wire Signal conversations to an agent group.
...
- **DM platform ID**: the phone number of the contact (e.g. `+15555550123`)
- **Group platform ID**: `group:<base64-groupId>` — find group IDs via `signal-cli -a +1YOURNUMBER listGroups`
