...
> Open the OneCLI web UI at http://127.0.0.1:10254, go to Apps, and click Connect for both Gmail and Google Calendar. Sign in with the Google account you want the agent to read.
...
> Open http://127.0.0.1:10254 and go to Agents. Click Manage on the agent that should have Gmail/Calendar access. Enable the Gmail and Google Calendar apps for this agent.
...
1. Clear SDK session for fresh tool discovery:
   ```bash
   find data/v2-sessions -name 'outbound.db' -exec sqlite3 {} "DELETE FROM session_state WHERE key='sdk_session_id';" \;
   ```

2. Stop any running agent containers and restart the service:
   ```bash
...
3. Ask the user to test: "check my calendar for this week" or "search my email for messages from [someone]"
